...
from seg_model.mrcnn import utils
...
def clip_boxes_graph(boxes, window):
    ...
    将一组边界框裁剪到指定的窗口内
    函数返回裁剪后的边界框张量 clipped，其形状为 [N, (y1, x1, y2, x2)]，其中每个边界框都被限制在指定的窗口内
    """
    # Split
...
class ProposalLayer(KL.Layer):
    ...
    输入：接收三个输入：RPN 输出的对象分数（rpn_probs）、边界框调整量（rpn_bbox）和锚点（anchors）。
    处理流程：
    选择 Top 分数的锚点：基于分数选择前 N 个锚点，以提高性能。
    应用边界框调整量：根据 RPN 的边界框调整量修改锚点的位置和大小。
    裁剪边界框：将调整后的边界框裁剪到图像范围内（0 到 1 的归一化坐标）。
    非最大抑制（NMS）：去除重叠的边界框，只保留最佳的候选区域
    """
...
class PyramidROIAlign(KL.Layer):
    ...
    constructor.
    对来自不同级别的特征金字塔的特征图执行 ROI 对齐操作
    在 call 方法中，根据边界框的大小和图像尺寸，为每个边界框选择最合适的特征图层级。
    对于每个选定的层级，裁剪出与边界框相对应的特征区域，并调整其大小以符合预定义的 pool_shape。
    将所有裁剪和调整后的特征拼接在一起，形成最终的输出张量。
    """
    ...
    def call(self, inputs):
        ...
        # 根据边界框的面积将每个 ROI 分配到特征金字塔的合适层级
        # 从边界框中提取坐标，并计算每个框的高度和宽度
        # roi_level 根据边界框的尺寸和图像的尺寸计算出来，确定每个边界框应该映射到哪个层级的特征图上
...
def overlaps_graph(boxes1, boxes2):
    ...
    计算两组边界框之间的交并比（Intersection over Union, IoU）
    """
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    #   is not even shared convolutional base of the RPN

    # 首先将feature map经过3*3卷积
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    ...
    weights.
    用于构建区域提议网络（Region Proposal Network, RPN）的 Keras 模型。这个函数封装了 RPN 的计算图，并允许在多次调用时共享其权重。
    允许在不同的输入特征图上重用相同的 RPN 计算图，从而使模型更加模块化和灵活
    anchors_per_location: number of anchors per pixel in the feature map
    anchor_stride: Controls the density of anchors. Typically 1 (anchors for
                   every pixel in the feature map), or 2 (every other pixel).
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    定义了区域提议网络（Region Proposal Network, RPN）的分类损失函数，用于训练 RPN 分类器
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    # 每个 ROI 可能属于多个类别之一，而模型为每个类别生成一个掩码预测。
    # tf.stack 将正样本的索引和相应的类别 ID 组合成一个二维数组。这个数组的每一行都包含一个正样本的索引和其对应的类别 ID。
    # 这个数组指定了我们应该从每个 ROI 的多个类别掩码预测中选择哪一个
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if config.GPU_COUNT > 1:
            from model.mrcnn.parallel_model import ParallelModel  # Todo:pass
    ...
    def detect(self, images, verbose=0):
        ...
        """
        # TODO : 在小批次图片检测时直接调用__call__方法加快速度，
        #        但是图执行模式无法将tensor类型转换为python原生类型
...
def norm_boxes_graph(boxes, shape):
    ...
    boxes: [..., (y1, x1, y2, x2)] in pixel coordinates
            排除型（Exclusive）坐标：在这种表示中，边界框的右下角坐标 (y2, x2) 是不包含在边界框内部的。
